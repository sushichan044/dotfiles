...
- **Data Access** (`log_id("cloudaudit.googleapis.com/data_access")`): API
  ...
  create, modify, or read user-provided _data_.
...
- `protoPayload.methodName`: The API method called.
  - _Gotcha:_ Prefer the exact match operator (`=`) when you know the exact
...
_any_ service (IAM, Storage, and others), GCP almost always puts this in
...
_newly_ added rather than just present in a broader update), you must also use
...
**Variables to replace:** `<USER_EMAIL>` _(Note: Change resource.type to
"folder" or "organization" for higher-level changes)_
...
**Variables to replace:** `<API_NAME>` _(Note: To find when a generic API was
disabled, use `audited_resource`, which differs from normal service endpoints.)_
