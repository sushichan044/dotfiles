...
When a user asks about Kubernetes _events_ like Pod evictions, Node scaling, or
BackOffs:
...
When auditing _infrastructure_ (for example, who created the GKE cluster
itself), this falls under general Google Cloud API audit logs. However, when
auditing _inside_ the cluster (for example, who deleted a Namespace or Pod):
