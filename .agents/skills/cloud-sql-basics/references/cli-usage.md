...
- **Restore from a backup:**
  ...
  - _To the same instance that took the backup:_

    ```bash
    gcloud sql backups restore backup_id --restore-instance=my-instance --quiet
    ```

  - _To a different target instance:_
    _(Requires `--backup-instance` to specify the source instance of the backup ID)_
    ```bash
