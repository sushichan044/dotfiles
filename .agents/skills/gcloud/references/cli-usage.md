...
_(Note: If `gcloud` was installed via a system package manager like APT or DNF,
use the system package manager to install components instead of `gcloud
components install`.)_
...
- **Service Account Key (Headless Automation):**
  ...
  _Security note: Restrict file permissions on JSON keys or prefer Workload
  Identity / Impersonation._
