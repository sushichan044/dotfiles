...
description: Design or revise narrative rhythm in Japanese chapters and essays, or diagnose dense but monotonous explanatory prose.
---
...
Use this skill for narrative prose where discovery is part of the requested reading
experience. For reports, answers, and operational instructions, state the main result
early and let the subject determine pacing. Preserve facts, necessary context, and
uncertainty while applying the relevant patterns.

For diagnosis, report passages and reasons. For writing or revision, finish when
the topic, leakage, tension, rhythm, and boundary checks are resolved. Additional
review rounds need a specific remaining defect.
...
2. **漏出テスト**：この規範の語彙・例文（「答えの半分」「緊張」「回収」「線を引く」「問いを〜返す」等）が本文にそのまま現れていないか検索する。Treat matches as review candidates. Preserve literal subject matter and the four permitted boundary forms; revise passages that merely announce a writing device.あわせて、節末に「次は〜」型の進行予告がないかを全節末で確認する。
